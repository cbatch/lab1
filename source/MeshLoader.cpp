...
namespace mshLdr
{
    ...
    Mesh::Mesh(std::string objectFile, meshNormalMode normalMode)
    {
        ...
        string err = tinyobj::LoadObj(shapes, materials, objectFile.c_str());
        if (!err.empty()) {
            std::cerr << err << endl;
        }
        resizeObj();
	...
	{
		for each (std::string fileName in objectFiles)
		{
			meshes.insert(std::pair<std::string, Mesh>(fileName.erase(fileName.length() - 4, fileName.length()), Mesh(fileName)));
		}
	}
    void MeshLoader::LoadMeshes(vector<std::string> objectFiles, meshNormalMode normalMode)
    {
        for each (std::string fileName in objectFiles)
        {
            meshes.insert(std::pair<std::string, Mesh>(fileName.erase(fileName.length() - 4, fileName.length()), Mesh(fileName, normalMode)));
        }
    }
